  ...
  responsive_images: {
    myTask: {
      ...
      files: [{
        ...
        custom_dest: 'tmp/custom_dest/width' <<< here might be different from orifinal copy cuz it caused some error. 
      }]
    }
  },
})
...
You can use width, height, name or path as a delimiter. <<< here, too. percent sign is the problem.

Please note that width, height and name are only available if they are set in the object literal that you use to set each generated size option. <<< here, too.

The path value contains additional directory structure from the current working directory (cwd in files array) to each image.  Using path allows any complex directory structure to persist into the rendered responsive images directory. <<< here, too.
